    ...
    <div class="container">
        <div class="row" v-if="subcategories.length > 0">
            ...
            <div class="col-md-9" v-else>
                <div class="jumbotron"><h1>Aucun Produit</h1></div>
            </div>
        </div>
        <div class="jumbotron" v-else><h1>Aucun Produit</h1></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        refreshSubcategories: function (id) {
            util.notify(
                "Actualisation Produits, veuillez patienter",
                "loading"
            );
            axios
                .get(data.getBaseURL() + "api/v1/user/subcategory?id=" + id)
