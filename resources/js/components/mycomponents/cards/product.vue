<template>
    <div class="card" :id="value.id">
        <img
            :src="storageURL + value.product_image"
            style="height: 200px; width: 250px"
            :alt="value.product_name"
        />
        <div style="padding-left: 15px">
            <h3 style="overflow: hidden">
                <b>{{ minify(value.product_name) }}</b>
            </h3>
            <h5>
                Prix :
                <span style="color: lime">{{
                    toCurrency(value.product_price)
                }}</span>
            </h5>
            <h5>Quantité : {{ value.product_quantity }}</h5>
        </div>
        <div
            class="input-group card-btn"
            @click="addToCart()"
            v-if="cart_quantity == 0"
        >
            <button class="btn btn-info card-btn">Ajouter au Panier</button>
        </div>
            ...
            <div class="input-group">
                ...
                <div class="input-group-btn">
                    <span
                        class="btn btn-info"
                        @click="addToCart()"
                        :class="{ disabled: value.product_quantity == 0 }"
                    >
...
export default {
    ...
    methods: {
        ...
        validate: function () {
            var preventMulticlick =
                this.time - this.lastClick < 3 && this.lastClick != 0;
